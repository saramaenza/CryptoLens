...
function Favorites({ favorites, onSelectCrypto, darkMode }) {
  ...
  return (
    <div>
      ...
      <hr className={`${darkMode ? "border-gray-400" : "border-gray-600"} mt-3`}></hr>
      <ul className="space-y-3 pt-5 text-md">
        {favorites.length === 0 && (
            <li className={`${darkMode ? "text-gray-600" : "text-gray-400"}`}>No favorites added</li>
        )}
        {favorites.map((crypto) => (
          <li
            key={crypto.id}
            className={
                `cursor-pointer flex items-center px-2 py-1 rounded transition-colors ` +
                (activeId === crypto.id
